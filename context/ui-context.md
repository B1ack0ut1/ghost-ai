...
Both fonts are exposed as CSS variables on the `<html>` element and applied to the app shell. In this workspace, the Geist files are loaded from bundled local assets via `next/font/local` so builds stay offline-safe, while preserving the same font family variables and visual treatment. The base `body` uses Geist Sans with `antialiased`.
